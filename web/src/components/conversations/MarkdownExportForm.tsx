  ...
  const copyRichText = async () => {
    ...
    try {
      ...
      if (typeof ClipboardItem !== "undefined" && navigator.clipboard?.write) {
        const htmlPromise = markdownPromise.then(richClipboardHtml);
        try {
          await navigator.clipboard.write([new ClipboardItem({
            "text/plain": markdownPromise.then((text) => new Blob([text], { type: "text/plain" })),
            "text/html": htmlPromise.then((html) => new Blob([html], { type: "text/html" })),
          })]);
          setNotice("Copied as rich text, with Markdown as the plain-text fallback.");
        } catch {
          await navigator.clipboard.writeText(await markdownPromise);
          setNotice("The browser rejected rich clipboard data; copied Markdown instead.");
        }
      } else {
        await navigator.clipboard.writeText(await markdownPromise);
        setNotice("Rich clipboard formats are unavailable in this browser; copied Markdown instead.");
      }
    } catch (reason) {
