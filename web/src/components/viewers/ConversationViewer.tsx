...
export default function ConversationViewer({
  ...
  const { canonicalInteractionIds, questionIds, questionResponses } = useMemo(() => {
    ...
    inferredInteractionResponses.forEach((item) => {
      if (responses.has(item.response.interaction_id)) return;
      responses.set(item.response.interaction_id, {
  ...
  const renderMessage = (msg: ConversationMessage) => {
    ...
    const hasPendingQuestion = Boolean(
      ...
      || msg.tool_calls?.some(
        (call) => call.interaction?.id && !questionResponses.has(call.interaction.id),
      ),
    );
    const hideWholeMessage = !hasPendingQuestion
      && !visibilityGroups.some((group) => visibility[group]);
    return (
      <div
        key={String(msg.id)}
      ...
      {pendingInteractions.length > 0 && (
        <section
          ...
          style={{
            ...
            background: "color-mix(in srgb, #F59E0B 7%, var(--aurora-surface-solid))",
          }}
        >
          <div style={{ display: "flex", alignItems: "center", gap: 7, color: "#A16207", fontSize: 11, fontWeight: 750 }}>
            <Icon name="inbox" size={14} />
            {t.dashboard.needsAttention}
            <span style={{ color: "var(--aurora-fg4)", fontWeight: 600 }}>
      ...
      >
        ...
        >
          ...
          <AssistantIdentityBadge
            ...
            serviceTier={identity?.service_tier}
            agentMode={identity?.agent_mode}
  ...
  if (role === "assistant") {
    ...
    const visibleToolCalls = toolCalls.filter((call) => (
      call.interaction
        ? (
            showAssistant
            || !questionResponses.has(call.interaction.id)
            || (showUser && questionResponses.has(call.interaction.id))
          )
        : showTools
    ));
