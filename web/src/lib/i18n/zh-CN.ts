const zhCN = {
  // Share links
  share: {
    title: "分享链接",
    targetTimeline: "项目时间线",
    targetDaily: "日报",
    targetMemory: "记忆",
    audiencePublic: "公开链接",
    audienceUser: "指定用户",
    recipient: "接收人",
    recipientHint: "只有该用户登录后才能查看,公开链接对所有人无效。",
    noRecipients: "没有可选用户",
    pickRecipient: "请选择接收人",
    directedTo: "已分享给",
    directedHint: "需该用户登录访问",
    expiresLabel: "有效期",
    days: "天",
    expiresHint: "0 或留空 = 永久",
    create: "生成分享链接",
    copy: "复制链接",
    copied: "已复制",
    copyFail: "复制失败",
    openInTab: "打开",
    revoke: "撤销",
    revokeConfirm: "撤销后链接立即失效,确定?",
    viewCount: "访问数",
    expiresAt: "过期",
    loadViews: "查看访问者",
    viewsLoadFail: "加载失败",
    noViews: "还没有人访问",
    when: "时间",
    from: "来自",
    ipAddr: "IP",
    shared: "分享",
    sharedBy: "分享自",
    revokedNotice: "此链接已被撤销。",
    expiredNotice: "此链接已过期。",
    publicWarn: "任何拿到链接的人都能查看内容。",
  },
  // Common
  loading: "加载中...",
  noData: "暂无数据",
  files: "个文件",
  file: "个文件",
  synced: "已同步",
  never: "从未",
  justNow: "刚刚",
  mAgo: "分钟前",
  hAgo: "小时前",
  dAgo: "天前",
  all: "全部",
  search: "搜索",
  login: "登录",
  logout: "退出",
  register: "注册",
  submit: "提交",
  cancel: "取消",
  back: "返回",
  forward: "前进",
  approve: "批准",
  online: "在线",
  viewAll: "查看全部",
  conversations: "对话",
  documents: "文档",
  projects: "项目",

  // Nav
  nav: {
    dashboard: "仪表板",
    tools: "工具",
    daily: "日报",
    search: "搜索",
    export: "导出",
    devices: "设备",
    admin: "管理",
    projects: "项目",
    memory: "记忆",
    pins: "已固定消息",
    inbox: "收到的分享",
  },

  inbox: {
    title: "收到的分享",
    subtitle: "其他用户分享给我的项目时间线、日报、记忆。",
    empty: "还没有收到任何分享。",
    open: "打开",
    sharedBy: "分享自",
    expiresAt: "过期",
    kindTimeline: "时间线",
    kindDaily: "日报",
    kindMemory: "记忆",
  },

  memoryShare: {
    button: "分享记忆",
  },

  // Dashboard
  dashboard: {
    title: "仪表板",
    overview: "总览",
    totalDocs: "总文件数",
    totalProjects: "总项目数",
    totalTools: "接入工具",
    totalDevices: "在线设备",
    toolOverview: "工具概览",
    projectOverview: "项目概览",
    recentActivity: "近期活动",
    noTools: "尚未同步任何工具，请启动采集器开始采集。",
    noActivity: "暂无活动记录",
    filesSynced: "{count} 个文件已同步",
    recentDocs: "最近文件",
    todayFiles: "今日同步",
    totalFiles: "总文件数",
    conversations: "对话",
    today: "今日",
    weeklyActivity: "近 7 日活动",
    openThreads: "打开的对话",
    openThreadsScope: "正在运行、最近 {minutes} 分钟内活跃，以及固定的对话",
    noOpenThreads: "连接的设备上没有打开的对话。",
    untitledThread: "未命名对话",
    openThreadsTruncated: "正在显示最相关的打开对话。固定对话可将其保留在这里。",
    pinThread: "固定对话",
    unpinThread: "取消固定对话",
    threadPinFailed: "无法更新此对话的固定状态。",
    healthHealthy: "健康",
    healthHandoffSoon: "即将交接",
    healthCritical: "立即交接",
    healthWarmingUp: "正在积累数据",
    healthNotReported: "未报告健康状态",
    healthStale: "已过期",
    needsAttention: "需要处理",
    recentConversations: "最近对话",
    clawDelegatedAgent: "Claw · {count} 个委派代理",
    clawDelegatedAgents: "Claw · {count} 个委派代理",
  },

  // Tools
  tools: {
    title: "所有工具",
    lastSync: "最后同步",
    filesCount: "{count} 个文件",
    noFiles: "该分类暂无文件",
    categories: "文件分类",
    projectsInTool: "相关项目",
    fileList: "文件列表",
    fileLoadFailed: "无法加载此工具的文件。",
    toolLoadFailed: "无法加载此工具。",
  },

  // Projects
  projectPage: {
    title: "所有项目",
    noProjects: "暂无项目",
    docsCount: "{count} 个文件",
    recentFiles: "最近文件",
    from: "来自",
    listLoadFailed: "无法加载项目。",
    loadFailed: "无法加载此项目的文件。",
    retry: "重试",
    loadMore: "加载更多（{loaded}/{total}）",
    loadingMore: "正在加载更多文件…",
  },

  // Documents
  document: {
    aiSummary: "AI 摘要",
    metadata: "元数据",
    contentInS3: "内容已存储至 S3（大文件）",
  },

  // Conversations
  conversation: {
    messages: "条消息",
    messagesTotal: "共 {total} 条消息，已加载 {loaded} 条",
    loadingMore: "加载更多消息...",
    loadEarlier: "加载更早的消息",
    loadMessageGap: "加载后续消息（剩余 {count} 条）",
    allLoaded: "所有消息已加载",
    displayOptions: "显示内容",
    displayUserMessages: "用户提示",
    displayAgentMessages: "助手消息",
    displayTools: "工具",
    displayTasks: "任务",
    displayAgents: "代理活动",
    displayThinking: "思考过程",
    displayContext: "上下文",
    displayShowAll: "全部显示",
    copyOmitRolePrefix: "复制时不含角色和时间",
    copyMessage: "复制",
    copyRichText: "富文本",
    copyMarkdown: "Markdown",
    copySlack: "Slack",
    copiedRichText: "已复制富文本",
    copiedMarkdown: "已复制 Markdown",
    copiedSlack: "已复制为 Slack 格式",
    copyFailed: "复制失败",
    pinMessage: "固定消息",
    unpinMessage: "取消固定消息",
    pinnedMessages: "已固定",
    pinnedMessage: "已固定消息",
    noPinnedMessages: "此对话中还没有已固定消息。",
    pinnedAcrossThreads: "你在所有对话中的个人书签。",
    line: "行",
    slackCopyTitle: "复制到 Slack",
    slackCopyHint: "复制格式化预览，然后粘贴到 Slack。如果 Android 去除了格式，请使用“全选”和浏览器的复制操作。",
    slackCopyAction: "复制格式化内容",
    slackCopySelectAction: "全选",
    slackCopyDone: "完成",
    slackCopySelected: "已选中预览 — 点复制或长按复制",
    slackCopyRichSuccess: "已复制格式化文本 — 请粘贴到 Slack。",
    slackCopyPlainFallback: "浏览器已提供可读的 Slack 文本回退格式。",
    slackCopyPreviewLabel: "Slack 格式化复制预览",
    thinking: "思考过程",
    showThinking: "展开思考",
    hideThinking: "收起思考",
    recoveredResponse: "已从元数据补回响应",
    thinkingFallback: "",
    offlineVscdbFallback: "已从本地 Antigravity 会话状态补回内容",
    offlinePbFallback: "已从本地 protobuf 缓存补回内容",
    collapse: "收起",
    branchedFrom: "分支自",
    tangent: "分支",
    tangents: "分支（{count}）",
    canvas: {
      chip: "画布",
      eyebrow: "Cursor 画布",
      close: "关闭画布查看器",
      copySource: "复制源代码",
      viewSource: "查看源代码",
      viewVisual: "查看可视化",
      openInCursor: "在 Cursor 中打开",
      openExternal: "在新标签页打开",
      interactive: "交互式",
      static: "静态",
      loading: "正在加载画布…",
      loadError: "画布内容加载失败。",
      unavailableTitle: "无法预览此画布",
      unavailableBody:
        "源设备尚未上传受支持的画布表示。下方显示了对话中记录的位置。",
      shelfTitle: "画布",
      shelfHint: "此对话中的最新画布",
      openCanvas: "打开画布",
      current: "已与源设备同步",
      awaitingCapture: "等待源设备上传",
    },
    expandAll: "展开全部",
    subagentDispatch: "子任务派发",
    parentAgent: "父代理",
    parentAgentDispatch: "任务派发",
    sessionContext: "会话上下文",
    additionalDirectives: "附加指令",
    additionalDirectivesHint: "此任务的产品与会话指导",
    scheduledAutomation: "定时自动化",
    attachments: "附件",
    imageAttachment: "图片",
    timeUnavailable: "时间不可用",
    sessionContextHint: "工具或工作区注入的上下文",
    recommendedPlugins: "推荐插件",
    webSearchContext: "网页搜索结果",
    pluginContext: "插件信息",
    uploadedDocuments: "上传的文档",
    activeGoalContext: "活动目标续接",
    conversationSummary: "对话摘要",
    workspaceInstructions: "工作区说明",
    interactiveQuestion: "交互式问题",
    permissionRequest: "权限请求",
    shellActivity: "Shell 命令",
    shellRunning: "运行中",
    shellCompleted: "已完成",
    shellFailed: "失败",
    shellCancelled: "已取消",
    shellElapsed: "已用时 {elapsed}",
    elicitationRequest: "外部输入请求",
    agentNeedsInput: "代理等待输入",
    answered: "已回答",
    resolved: "已处理",
    resolvedAnswerUnavailable: "Claude Code 未记录所选择的选项。",
    awaitingResponse: "等待回答",
    backgroundTasksRunning: "{count} 个后台任务运行中",
    backgroundTag: "后台",
    planMode: "计划模式",
    cancelled: "已取消",
    chooseOne: "选择一项",
    chooseMany: "可选择多项",
    freeResponse: "自由回答",
    yourResponse: "你的回答",
    customResponse: "自定义回答",
    notes: "备注",
    diagnostics: "导出诊断",
    diagnosticsHelp: "用于判断会话缺失发生在 LS、元数据还是本地缓存层。",
    stepFetchFailed: "步骤抓取失败",
    pbShellOnly: "只剩加密 .pb 外壳",
    truncated: "元数据消息被 4MB 限制截断",
    plannerResponses: "planner response",
    thinkingOnly: "只有 thinking",
    metadataRecovered: "metadata 补回",
    transcriptRecovered: "transcript 补回",
    offlineVscdbRecovered: "会话状态补回",
    chatExportRecovered: "导出 Markdown 补回",
    offlinePbRecovered: "protobuf 补回",
    brainArtifacts: "brain 文件",
    browserFrames: "浏览器截图",
    browserHighlights: "浏览器高亮",
    lowActivity: "低活动对话",
    lowActivityHint: "空白、单向或非常短的对话收纳在这里。",
    promptNavigator: "提示词导航",
    latestAgentMessage: "最新助手消息",
    promptCount: "共 {count} 条提示词",
    filteredPromptCount: "显示 {visible} / {total} 条提示词",
    searchPrompts: "搜索 {count} 条提示词",
    noMatchingPrompts: "没有匹配的提示词",
    searchMessages: "搜索此对话中的所有消息",
    clearSearch: "清除对话搜索",
    matchingMessages: "{count} 条匹配消息",
    noMatchingMessages: "没有匹配的消息",
    recentSearches: "最近搜索",
    closePromptNavigator: "关闭提示词导航",
    promptFallback: "提示词 {number}",
    lastActivity: "最近活动",
    projectLocation: "项目位置",
    copyProjectLocation: "复制项目位置：{host}:{path}",
    projectLocationCopied: "项目位置已复制",
    projectLocationCopyFailed: "无法复制项目位置",
    resumeWith: "继续使用",
    copyResumeCommand: "复制继续会话命令",
    resumeCommandCopied: "继续会话命令已复制",
    resumeCommandCopyFailed: "无法复制继续会话命令",
    diagram: "图表",
    renderingDiagram: "正在渲染图表...",
    diagramError: "无法渲染此图表，源代码保留在下方。",
    diagramTooLarge: "此图表过大，无法安全渲染，源代码保留在下方。",
    viewDiagramSource: "查看图表源代码",
  },

  // Daily
  daily: {
    title: "每日活动",
    noActivity: "暂无活动记录",
    filesSynced: "{count} 个文件已同步",
    // daily/[date] detail page
    subtitle: "{conversations} 对话 · {tools} 工具 · {messages} 消息",
    generate: "生成 AI 日报总结",
    regenerate: "重新生成",
    generating: "AI 摘要中…",
    generateFailed: "生成失败",
    aiSummaryChip: "AI 日报总结",
    toolUsage: "工具使用",
    noToolMessages: "当天无对话消息记录",
    conversationsHeader: "对话 ({count})",
    noConversations: "当天没有对话记录",
    moreItems: "还有 {count} 个",
    keyChanges: "关键变更 ({count})",
    expandFlow: "展开当日对话回顾",
    flowHeader: "当日对话回顾 ({count})",
    collapse: "收起",
    noMessages: "当天无对话消息",
    loadMore: "加载更多（{loaded}/{total}）",
  },

  // Search
  searchPage: {
    title: "搜索",
    subtitle: "搜索所有对话消息或同步文件",
    scopeLabel: "搜索范围",
    conversations: "对话",
    files: "文件与笔记",
    placeholder: "搜索所有 AI 工具数据...",
    messagePlaceholder: "搜索所有用户和助手消息...",
    allTools: "所有工具",
    results: "找到 {total} 条结果，关键词 \"{query}\"",
    noResults: "未找到结果",
    you: "你",
    assistant: "助手",
    fuzzyMatch: "模糊匹配",
    subagentMatch: "子代理",
    loadMore: "加载更多结果",
  },

  // Devices
  devices: {
    title: "设备",
    subtitle: "所有已注册的采集器实例",
    noDevices: "尚未注册任何设备",
    installHint: "在任意设备上安装采集器：",
    registered: "注册于",
    snippetCollectorOnly: "只装采集器",
    snippetMetaOption: "或一条命令装齐 collector + MCP memory:",
    delete: "删除",
    deleteHint: "删除设备和它名下的所有数据(不可撤销)",
    deleteConfirm: "确认删除「{name}」?会一并清掉它名下的 {count} 个文件和所有附带数据,不可撤销。\n\n常用于撤销一次导入(把对应的「Imported …」设备删掉即可)。",
    deleteFailed: "删除失败",
    newSession: "新建 Codex 会话",
    newSessionHint: "在此设备上启动受管 Codex 会话",
  },
  control: {
    managed: "受管会话",
    managedStarting: "受管 · 启动中",
    managedFailed: "受管 · 已失败",
    agentWorking: "智能体正在工作…",
    interrupt: "中断",
    endControl: "结束控制",
    send: "发送",
    steer: "插入指令",
    composerPlaceholder: "向该智能体发送消息…",
    steerPlaceholder: "向当前回合插入指令…",
    answer: "回答",
    customAnswerPlaceholder: "自定义回答…",
    needsInput: "智能体需要输入",
    approve: "批准",
    approveForSession: "本会话内批准",
    decline: "拒绝",
    commandApproval: "命令执行审批",
    fileChangeApproval: "文件修改审批",
    permissionRequest: "权限请求",
    networkAccess: "网络访问",
    writeAccess: "写入",
    readAccess: "读取",
    commandFailed: "命令失败",
    approvedNotice: "命令已批准。",
    declinedNotice: "命令已拒绝——未执行。",
    answeredNotice: "回答已发送。",
    dialogTitle: "新建受管 Codex 会话",
    cwdLabel: "工作目录（可选）",
    modelLabel: "模型（可选）",
    modelPlaceholder: "使用设备默认模型",
    approvalPolicyLabel: "审批策略（可选）",
    sandboxLabel: "沙箱（可选）",
    agentDefaultOption: "智能体默认",
    firstMessageLabel: "首条消息（可选）",
    firstMessageLabelRequired: "首条消息（必填）",
    firstMessageRequired: "必须填写首条消息——智能体执行首个回合前会话没有记录可绑定。",
    firstMessagePlaceholder: "希望智能体先做什么？",
    start: "启动会话",
    close: "关闭",
    starting: "正在启动受管会话…",
    waitingAgent: "等待智能体启动…",
    waitingTranscript: "智能体已运行 — 等待其对话记录同步…",
    backgroundStart: "会话正在后台启动；对话记录同步后将出现在列表中。",
    startFailed: "启动会话失败",
    sessionFailedPrefix: "会话失败：",
  },

  // Auth
  auth: {
    loginTitle: "登录",
    registerTitle: "注册",
    email: "邮箱",
    password: "密码",
    rememberMe: "记住我",
    rememberMeHint: "在此设备和应用更新后保持登录状态。",
    name: "姓名（选填）",
    noAccount: "没有账号？",
    hasAccount: "已有账号？",
    registerSuccess: "注册成功！",
    pendingApproval: "您的账号正在等待管理员审批。",
    goToLogin: "前往登录",
    invalidCredentials: "登录失败",
    ownerWelcome: "你是第一个注册的用户，已自动设为 owner。",
    memberWelcome: "注册完成,账号已激活。下面是你的专属 Collector Token,用它即可在任意设备上开始采集。",
    collectorTokenLabel: "你的 Collector Token",
    tokenSaveHint: "请现在复制保存。用这个 token 在任何设备上 memento-collector setup 即可。丢了也没关系，随时可以在个人资料页重新生成。",
    registrationClosed: "注册已关闭。请联系管理员获取邀请码。",
    inviteCodeRequired: "邀请码(必填)",
    inviteCodeOptional: "邀请码(选填,免审批直接激活)",
    continueWithGithub: "使用 GitHub 登录",
    orContinueWith: "或",
    signingIn: "正在登录…",
    githubOauthFailed: "GitHub 登录失败,请重试",
    accountDisabled: "账号未激活或已停用",
    returningToApp: "正在返回 Memento…",
    continueInBrowser: "已在浏览器中打开,请在那里继续",
    desktopHandoffFailed: "无法把登录信息交回桌面端,请重新登录",
    viewSplash: "了解 Memento",
  },

  // Token display + profile
  common: {
    copy: "复制",
    copied: "已复制",
    copyFailed: "复制失败",
    copySelected: "复制被拦截。文本已选中，请使用浏览器的复制操作。",
    reveal: "显示",
    hide: "隐藏",
    cancel: "取消",
    confirm: "确认",
    viewFullDocument: "查看完整文档 →",
  },
  profile: {
    title: "个人资料",
    subtitle: "账号信息与备份",
    role: "角色",
    status: "状态",
    email: "邮箱",
    name: "姓名",
    collectorToken: "Collector Token",
    noToken: "当前账号暂未分配 token（账号尚未激活）",
    rotate: "重新生成",
    rotateHint: "重新生成后，所有正在使用旧 token 的采集器会立即失效，需要用新 token 重新 setup。",
    rotateConfirm: "确认重新生成 collector token？所有在用采集器需重新配置。",
    rotateSuccess: "已重新生成，请复制新 token 并到各采集器重新 setup。",
    logout: "退出登录",
    backup: "备份与导入",
    backupDesc: "把你的所有数据(文档、对话、知识图谱、日报、分享链接)打包成一个 .zip,用来本地存档或迁移到另一个 Memento 实例。",
    exportBtn: "导出全部数据",
    exporting: "打包中…",
    includeLogs: "包含审计日志(IP 已粗化到 /24)",
    restoreDesc: "从之前导出的 .zip 恢复。会在本账号下新增一台「Imported on …」设备,所有数据挂在它名下;后悔的话删掉这台设备就回滚了。",
    importBtn: "从备份导入",
    importing: "恢复中…",
    importPick: "选择 .zip 文件",
    importSuccess: "导入成功",
    importWarnings: "导入告警",
  },

  // Admin
  admin: {
    title: "管理",
    syncStatus: "同步状态",
    users: "用户",
    devices: "设备管理",
    neverSynced: "从未同步",
    requireAdmin: "需要管理员权限，请使用管理员账号登录。",
    purgeDevice: "清空数据",
    purgeConfirm: "确定要清空该设备的所有数据吗？此操作不可逆！",
    purgeSuccess: "已清空 {name}，删除 {count} 个文件",
    resync: "重新采集",
    updateCollector: "更新采集器",
    updateSuccess: "已通知 {name} 更新采集器",
    collectorUpToDate: "已最新",
    updateToVersion: "更新到 v{version}",
    updateNotifiedVersion: "已通知 {name} 升级到 v{version}",
    resyncConfirm: "确定要清空该设备的所有数据并触发重新采集吗？",
    resyncSuccess: "已通知 {name} 重新采集",
    deleteDevice: "删除设备",
    deleteConfirm: "确定要删除设备 {name} 吗？\n\n将彻底清除：设备记录、所有文档/对话/嵌入、相关知识图谱观察、孤立项目。此操作不可逆！",
    deleteSuccess: "已删除设备 {name}（清除 {count} 个文档）",
    docsCount: "{count} 个文件",
    // Memory maintenance
    memoryVacuum: "清理孤儿记忆",
    memoryVacuumHint: "清掉观察记录已被删光的僵尸实体和它们的关系（不影响活跃实体）。",
    memoryVacuumConfirm: "确定要清理零观察记录的记忆实体吗？",
    memoryVacuumSuccess: "已清理 {ents} 个实体 + {rels} 条关系",
    // Tabs
    tabUsers: "用户",
    tabDevices: "设备",
    tabInvites: "邀请码",
    tabPermissions: "授权",
    tabAudit: "审计日志",
    tabSync: "同步状态",
    // Users — row actions
    userRole: "角色",
    userStatus: "状态",
    userDelete: "删除用户",
    userDeleteConfirm: "确定删除 {email}？将级联清除 ta 的所有设备、文档、记忆图谱。此操作不可逆。",
    userDeleteSuccess: "已删除 {email}",
    userResetToken: "重置 token",
    userResetTokenConfirm: "为 {email} 重置 collector token？ta 的所有采集器会立即失效。",
    userResetTokenSuccess: "已重置 {email} 的 token",
    userTransferOwner: "转让 owner 给 ta",
    userTransferOwnerConfirm: "把 owner 转让给 {email}?你会变成 admin。",
    userTransferOwnerSuccess: "已把 owner 转让给 {email}",
    userUpdateSuccess: "已更新 {email}",
    // Invites
    invitesTitle: "邀请码",
    invitesSubtitle: "生成邀请码,被邀请人注册时填入即可直接激活,免审批",
    inviteCreate: "创建邀请码",
    inviteMaxUses: "可用次数",
    inviteExpiresDays: "有效天数",
    inviteRoleOnAccept: "激活后角色",
    inviteNote: "备注",
    inviteNeverExpires: "永不过期",
    inviteRevoke: "撤销",
    inviteRevokeConfirm: "撤销这个邀请码?未使用的用户将无法再用它注册。",
    inviteUsage: "{used}/{max} 次",
    invitesEmpty: "还没生成过邀请码",
    // Permissions
    permsTitle: "细粒度授权",
    permsSubtitle: "给 viewer 按 project 或 tool 单独发 read/write 权限",
    permsGrant: "新建授权",
    permsGrantPlaceholderProject: "project_id(可选)",
    permsGrantPlaceholderTool: "tool_id(可选)",
    permsPermission: "权限",
    permsRevoke: "撤销",
    permsRevokeConfirm: "撤销这条授权?",
    permsEmpty: "还没授权记录",
    // Audit
    auditTitle: "审计日志",
    auditSubtitle: "谁在什么时候访问了哪个文档",
    auditUserId: "用户",
    auditAction: "动作",
    auditDocument: "文档",
    auditIp: "IP",
    auditTime: "时间",
    auditEmpty: "还没有日志",
    auditLoadMore: "加载更多",
    auditFilterUser: "按用户 ID 过滤",
    auditFilterAction: "按动作过滤",
  },

  // Timeline
  timeline: {
    title: "项目时间线",
    subtitle: "AI 思考过程还原",
    noEvents: "暂无时间线事件",
    expandSession: "展开对话",
    messagesCount: "{count} 条消息",
    viewConversation: "查看完整对话",
    viewDocument: "查看文档",
    allCategories: "全部类型",
    loadMore: "加载更多",
    allLoaded: "已加载全部事件",
    oldToNew: "从旧到新",
    newToOld: "从新到旧",
  },

  // App
  app: {
    title: "Memento",
    subtitle: "AI 编程记忆",
    version: "v0.1.0",
  },

  // Landing page (/ )
  landing: {
    nav_features: "特性",
    nav_tools: "支持工具",
    nav_install: "安装",
    nav_github: "GitHub",
    cta_login: "登录",
    cta_dashboard: "进入面板",
    cta_install: "一键安装",
    hero_title: "你的 AI 编程记忆,一个地方找到",
    hero_sub: "自动采集 Claude Code / Codex / Cursor / Hermes / Obsidian 等 7+ 工具的对话、计划、记忆文件,本地部署,跨设备同步。",
    stats_files: "个文件",
    stats_devices: "台设备",
    stats_tools: "个工具",
    stats_messages: "条对话消息",

    features_title: "特性",
    features_sub: "为多工具、多设备的 AI 编程流而造",
    f1_title: "跨设备自动同步",
    f1_desc: "守护进程 FSEvents/inotify 监听 AI 工具数据目录,增量上传,断网离线队列、自愈重试",
    f2_title: "知识图谱",
    f2_desc: "LLM 自动从对话抽实体(项目 / 工具 / 技术 / 人物 / 概念)、关系、观察;7 天后老观察自动压缩为 summary",
    f3_title: "混合检索 + MCP",
    f3_desc: "BGE-M3 语义向量 + jieba 分词全文索引,中英文都能搜;MCP 让 Claude Code / Cursor / Codex 等直接查记忆",
    f4_title: "AI 日报",
    f4_desc: "Celery 每天 23:30 两阶段:逐文档生成摘要 → 跨工具聚合,记得自己今天干了什么",
    f5_title: "入队前脱敏",
    f5_desc: "Collector 本地就过 14 类密钥正则(OpenAI / Anthropic / GitHub / Slack / AWS / 私钥 / Bearer / URL 内嵌凭证 …),磁盘队列也安全",
    f6_title: "完全自托管",
    f6_desc: "Docker Compose 一键起 Postgres + Redis + MinIO,数据不出机器;多租户隔离 + 细粒度授权 + 审计日志",

    tools_title: "统一采集这些工具",
    tools_sub: "按工具分类,按项目组织,按对话还原",
    tools_col_name: "工具",
    tools_col_content: "采集内容",
    tools_col_format: "格式",

    how_title: "工作原理",
    how_sub: "三层, 各司其职",
    how_1_title: "Collector",
    how_1_desc: "装在每台机器上的 Python 守护进程,监听 AI 工具的数据目录,只上传增量",
    how_2_title: "Server",
    how_2_desc: "FastAPI + PostgreSQL + Redis,索引、搜索、AI 总结;MinIO 存大文件",
    how_3_title: "Web + MCP",
    how_3_desc: "浏览器按设备→工具→项目→对话导航;MCP 让 AI IDE 直接查记忆",

    install_title: "一键安装",
    install_sub: "macOS / Linux / Windows,5 分钟起栈",
    install_macos: "macOS · Linux",
    install_windows: "Windows",
    install_copy: "复制",
    install_copied: "已复制",
    install_note: "脚本会生成随机密钥、起 Docker 栈、建首用户、装采集器。需要 Docker + Python 3.11+",

    desktop_title: "桌面客户端",
    desktop_sub: "不想碰命令行?下载客户端 —— 内置采集器 + MCP,免装 Python,自动更新",
    desktop_macos: "macOS",
    desktop_windows: "Windows",
    desktop_linux: "Linux",
    desktop_macos_ext: ".dmg (Apple Silicon)",
    desktop_windows_ext: ".exe (安装版)",
    desktop_linux_ext: ".AppImage / .deb",
    desktop_btn: "下载",
    desktop_note: "首次打开在 Server 标签填服务器地址 + token(没有账号可直接在 app 内注册)即可开始采集,全程不开浏览器。习惯命令行的可继续用下方 pip 方式。",

    bootstrap_kicker: "项目记忆迁移",
    bootstrap_title: "把一个项目的完整记忆喂给新开的 Claude Code / Codex",
    bootstrap_subtitle: "你已经在 Memento 里攒了几百条对话和计划。新启动一个 AI 项目时,把这些上下文一键交接过去 —— 不用复制粘贴。",
    bootstrap_path_live_title: "MCP 直连 (推荐)",
    bootstrap_path_live_subtitle: "新项目接 memento-memory MCP,需要时按需拉,还能继续写新观察。装了桌面客户端就已经自动配好;pip 用户 `memento-collector setup` 一遍即可。",
    bootstrap_path_live_b1: "在新项目根目录建 CLAUDE.md / AGENTS.md,粘下面这段。",
    bootstrap_path_live_b2: "下次 claude/codex 进这个目录,它会主动调 memory_context 把项目记忆灌进来。",
    bootstrap_path_live_b3: "AI 还能调 memory_store 反向写回新观察 —— 记忆在两个项目之间互通。",
    bootstrap_path_dump_title: "一次性 Markdown 导出",
    bootstrap_path_dump_subtitle: "适合离线场景、给同事发整段背景、或不想接 MCP 的轻量用法。Memento 会把项目的所有 memory / plan / identity 文件 + 知识图谱打包成单个 .md。",
    bootstrap_path_dump_b1: "Web 进 /projects/<id>,右上角点「导出 .md」。",
    bootstrap_path_dump_b2: "把下载的 memento-context-*.md 拷到新项目里改名 MEMENTO-CONTEXT.md。",
    bootstrap_path_dump_b3: "CLAUDE.md 里加一句「Read MEMENTO-CONTEXT.md before starting」。",
    bootstrap_path_dump_tip: "导出文件只是冷启动用,不会自动跟上你之后的新对话。需要持续同步,选左侧的 MCP 路径。",

    footer_download: "下载客户端",
    footer_tagline: "给你的 AI 工具建个共享大脑 · 本地优先 · 开源",
    footer_docs: "文档",
    footer_api: "API 文档",
    footer_install: "安装说明",
    footer_github: "GitHub",
  },

  // Project detail / list page
  projects_page: {
    exportMd: "导出 .md",
    exportMdHint: "把项目的全部记忆/计划/身份卡 + 知识图谱打包成一个 Markdown 文件,用来喂给新开的 AI 项目",
    exportFailed: "导出失败",
  },

  // Categories
  category: {
    config: "配置",
    conversation: "对话",
    memory: "记忆",
    history: "历史",
    plan: "计划",
    skill: "技能",
    identity: "身份",
    learning: "学习",
    debug: "调试",
    extension: "扩展",
    note: "笔记",
    file_history: "文件历史",
    state: "状态",
  },
};

export default zhCN;

type DeepStringify<T> = {
  [K in keyof T]: T[K] extends Record<string, unknown> ? DeepStringify<T[K]> : string;
};

export type Translations = DeepStringify<typeof zhCN>;
